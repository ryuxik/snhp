"""The three arms behind one small interface.

  sticker/1 — the cultural control: tag price, take it or leave it, with the
              LES gut ritual (−20% every 30 unsold days). No engine.
  hazard/1  — ablation for H-V3: the ENGINE'S learned per-item hazard drives
              computed markdowns (weekly re-solve), but offers don't exist.
  offer/1   — make-an-offer: asks stay at tag; the engine accepts/counters
              against the event-consistent waiting value.
...
from vintage.calibration import (F_EWMA, F_INIT, MARKDOWN_AGE,
                                 MARKDOWN_FACTOR, REPRICE_EVERY)
from vintage.engine import Beliefs, decide_offer, solve_price
...
@dataclass
class OfferPolicy:
    """Make-an-offer. The ask never moves (the offer flow IS the markdown
    channel — flagged in results); the engine's waiting value assumes
    future settlement at f̂ x ask, where f̂ is the EWMA of realized
    (price / ask) over THIS arm's own sales — the learner may not assume a
    sticker world its own mechanism abolished."""
